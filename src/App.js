...
export default function App() {
  return (
    <Canvas concurrent gl={{ alpha: false }} pixelRatio={[1, 1.5]} camera={{ position: [0, 3, 100], fov: 15 }}>
      <fog attach="fog" args={['blue', 1, 100]} />
      <Suspense fallback={null}>
        ...
        <directionalLight color="blue" position={[-50, 0, -40]} intensity={2} />
        <Intro />
      </Suspense>
    </Canvas>
  )
...
function VideoText(props) {
  const [video] = useState(() => Object.assign(document.createElement('video'), { src: '/thunder.mp4', crossOrigin: 'Anonymous', loop: true, muted: true }))
